...
/**
 * @Title("MC-15616: Verify Secure URLs For Storefront Downloadable Pages")
 * @Description("Verify that the Secure URL configuration applies to the Downloadable pages on the Storefront<h3>Test files</h3>app/code/Magento/Downloadable/Test/Mftf/Test/StorefrontVerifySecureURLRedirectDownloadableTest.xml<br>")
 * @TestCaseId("MC-15616")
 * @group downloadable
 * @group configuration
 * @group secure_storefront_url
 */
class StorefrontVerifySecureURLRedirectDownloadableTestCest
{
	/**
	 ...
	 * @throws \Exception
	 */
	public function StorefrontVerifySecureURLRedirectDownloadableTest(AcceptanceTester $I)
	{
		$hostname = $I->executeJS("return window.location.host"); // stepKey: hostname
		$I->amOnUrl("http://{$hostname}/downloadable/customer"); // stepKey: goToUnsecureDownloadableCustomerURL
		$I->seeCurrentUrlEquals("https://{$hostname}/downloadable/customer"); // stepKey: seeSecureDownloadableCustomerURL
		$I->amOnUrl("http://{$hostname}/downloadable/download"); // stepKey: goToUnsecureDownloadableDownloadURL
		$I->seeCurrentUrlEquals("https://{$hostname}/downloadable/download"); // stepKey: seeSecureDownloadableDownloadURL
	}
}
