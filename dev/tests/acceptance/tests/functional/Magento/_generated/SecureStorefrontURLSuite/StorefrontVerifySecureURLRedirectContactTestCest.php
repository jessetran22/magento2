...
/**
 * @Title("MC-15539: Verify Secure URLs For Storefront Contact Pages")
 * @Description("Verify that the Secure URL configuration applies to the Contact pages on the Storefront<h3>Test files</h3>app/code/Magento/Contact/Test/Mftf/Test/StorefrontVerifySecureURLRedirectContactTest.xml<br>")
 * @TestCaseId("MC-15539")
 * @group contact
 * @group configuration
 * @group secure_storefront_url
 */
class StorefrontVerifySecureURLRedirectContactTestCest
{
	/**
	 ...
	 * @throws \Exception
	 */
	public function StorefrontVerifySecureURLRedirectContactTest(AcceptanceTester $I)
	{
		$hostname = $I->executeJS("return window.location.host"); // stepKey: hostname
		$I->amOnUrl("http://{$hostname}/contact"); // stepKey: goToUnsecureContactURL
		$I->seeCurrentUrlEquals("https://{$hostname}/contact"); // stepKey: seeSecureContactURL
		$I->amOnUrl("http://{$hostname}/contact/index/post"); // stepKey: goToUnsecureContactFormURL
		$I->seeCurrentUrlEquals("https://{$hostname}/contact/index/post"); // stepKey: seeSecureContactFormURL
	}
}
