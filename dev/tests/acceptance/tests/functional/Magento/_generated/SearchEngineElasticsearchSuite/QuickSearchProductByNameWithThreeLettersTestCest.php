...
/**
 * @Title("MC-15034: User should be able to use Quick Search to find products by their first three letters")
 * @Description("Use Quick Search to find a product using only first three letters<h3>Test files</h3>app/code/Magento/CatalogSearch/Test/Mftf/Test/SearchEntityResultsTest/QuickSearchProductByNameWithThreeLettersTest.xml<br>")
 * @TestCaseId("MC-15034")
 * @group CatalogSearch
 * @group SearchEngineElasticsearch
 * @group mtf_migrated
 */
class QuickSearchProductByNameWithThreeLettersTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function QuickSearchProductByNameWithThreeLettersTest(AcceptanceTester $I)
	{
		...
		$getFirstThreeLetters = $I->executeJS("var s = '" . $I->retrieveEntityField('createSimpleProduct', 'name', 'test') . "'; var ret=s.substring(0,3); return ret;"); // stepKey: getFirstThreeLetters
		$I->comment("Entering Action Group [searchStorefront] StorefrontCheckQuickSearchStringActionGroup");
		$I->fillField("#search", $getFirstThreeLetters); // stepKey: fillInputSearchStorefront
		...
		$I->seeInTitle("Search results for: '{$getFirstThreeLetters}'"); // stepKey: assertQuickSearchTitleSearchStorefront
		$I->see("Search results for: '{$getFirstThreeLetters}'", ".page-title span"); // stepKey: assertQuickSearchNameSearchStorefront
