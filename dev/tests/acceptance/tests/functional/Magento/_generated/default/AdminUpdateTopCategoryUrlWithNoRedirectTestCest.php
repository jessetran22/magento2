...
/**
 * @Title("MC-6056: Update top category url and do not create redirect")
 * @Description("Login as admin and update top category url and do not create redirect<h3>Test files</h3>app/code/Magento/Catalog/Test/Mftf/Test/AdminUpdateTopCategoryUrlWithNoRedirectTest.xml<br>")
 * @TestCaseId("MC-6056")
 * @group Catalog
 * @group mtf_migrated
 */
class AdminUpdateTopCategoryUrlWithNoRedirectTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function AdminUpdateTopCategoryUrlWithNoRedirectTest(AcceptanceTester $I)
	{
		...
		$I->comment("Update category UrlKey and uncheck permanent redirect for old URL");
		...
		$I->fillField("input[name='url_key']", "updatedurl"); // stepKey: updateUrlKey
		$I->uncheckOption("[data-index='url_key_create_redirect'] input[type='checkbox']"); // stepKey: uncheckPermanentRedirectCheckBox
		...
		$I->fillField(".admin__data-grid-filters input[name='request_path']", "updatedurl"); // stepKey: fillUpdatedUrlInRedirectPathFilter
		...
		$I->see($I->retrieveEntityField('createDefaultCategory', 'name', 'test') . "/" . $I->retrieveEntityField('createTwoLevelNestedCategories', 'name', 'test') . "/updatedurl.html", "//*[@data-role='grid']//tbody//tr[1+1]//td[count(//*[@data-role='grid']//th[contains(., 'Request Path')]/preceding-sibling::th)+1]"); // stepKey: seeTheRedirectPath
		$I->comment("Verify third level category's old URL path doesn't show redirect path");
		$I->comment("Entering Action Group [clickOnResetButton1] AdminClearGridFiltersActionGroup");
		$I->click(".admin__data-grid-header [data-action='grid-filter-reset']"); // stepKey: clickClearFiltersClickOnResetButton1
		$I->waitForPageLoad(30); // stepKey: clickClearFiltersClickOnResetButton1WaitForPageLoad
		$I->waitForPageLoad(30); // stepKey: waitForPageLoadedClickOnResetButton1
		$I->comment("Exiting Action Group [clickOnResetButton1] AdminClearGridFiltersActionGroup");
		...
		$I->fillField(".admin__data-grid-filters input[name='request_path']", "thirdlevel" . msq("Three_nested_categories")); // stepKey: fillOldUrlInRedirectPathFilter
		...
		$I->see("We couldn't find any records.", ".data-grid-tr-no-data td"); // stepKey: seeEmptyRecodsMessage
	}
}
