...
/**
 * @Title("MC-236: Customer should only see cart price rule discount if condition total items quantity greater than")
 * @Description("Customer should only see cart price rule discount if condition total items quantity greater than<h3>Test files</h3>app/code/Magento/SalesRule/Test/Mftf/Test/StorefrontCartPriceRuleQuantityTest.xml<br>")
 * @TestCaseId("MC-236")
 * @group SalesRule
 */
class StorefrontCartPriceRuleQuantityTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function StorefrontCartPriceRuleQuantityTest(AcceptanceTester $I)
	{
		...
		$I->selectOption("select[data-form-part='sales_rule_form'][data-ui-id='newchild-0-select-rule-conditions-1-new-child']", "Total Items Quantity"); // stepKey: selectProductAttributes
		$I->waitForPageLoad(30); // stepKey: waitForConditions
		$I->click("//*[@id='conditions__1__children']/li[1]/span[1]/a"); // stepKey: startEditOperator
		$I->selectOption("#conditions__1--1__operator", "greater than"); // stepKey: fillOperator
		$I->click("//*[@id='conditions__1__children']/li[1]/span[2]/a"); // stepKey: startEditValue
		$I->fillField("#conditions__1--1__value", "1"); // stepKey: fillValue
		...
		$I->fillField("input[name='discount_amount']", "1.00"); // stepKey: fillDiscountAmount
		...
		$reindexInvalidatedIndices = $I->magentoCron("index", 90); // stepKey: reindexInvalidatedIndices
		$I->comment($reindexInvalidatedIndices);
		$I->comment("Add 1 product to the cart");
		...
		$I->comment("Should not see the discount yet because we have only 1 item in our cart");
		...
		$I->comment("Add the same product to the cart again (2 total)");
		$I->amOnPage($I->retrieveEntityField('createPreReqProduct', 'custom_attributes[url_key]', 'test') . ".html"); // stepKey: goToProductPage2
		...
		$I->comment("Now we should see the discount because we have more than 1 item");
		...
		$I->see("-$1.00", "td[data-th='Discount']"); // stepKey: assertDiscountSeeDiscountTotal
