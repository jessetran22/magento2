...
/**
 * @Title("MC-89: customer should be taken to the product details page when clicking add to cart")
 * @Description("customer should be taken to the product details page when clicking add to cart<h3>Test files</h3>app/code/Magento/ConfigurableProduct/Test/Mftf/Test/StorefrontConfigurableProductViewTest/StorefrontConfigurableProductAddToCartTest.xml<br>")
 * @TestCaseId("MC-89")
 * @group ConfigurableProduct
 */
class StorefrontConfigurableProductAddToCartTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function StorefrontConfigurableProductAddToCartTest(AcceptanceTester $I)
	{
		$I->comment("Should be taken to product details page when adding to cart because an option needs to be selected");
		$I->amOnPage("/" . $I->retrieveEntityField('createCategory', 'custom_attributes[url_key]', 'test') . ".html"); // stepKey: amOnCategoryPage1
		$I->waitForPageLoad(30); // stepKey: wait1
		$I->click("#mode-list"); // stepKey: clickListView
		$I->waitForPageLoad(30); // stepKey: wait2
		$I->comment("Entering Action Group [clickAddToCart] StorefrontClickAddToCartButtonActionGroup");
		$I->click("button.action.tocart.primary"); // stepKey: addToCartClickAddToCart
		$I->waitForPageLoad(30); // stepKey: waitAddToCartClickAddToCart
		$I->comment("Exiting Action Group [clickAddToCart] StorefrontClickAddToCartButtonActionGroup");
		$grabUrl = $I->grabFromCurrentUrl(); // stepKey: grabUrl
		$I->assertStringContainsString("testproductname" . msq("_defaultProduct"), $grabUrl); // stepKey: assertUrl
	}
}
