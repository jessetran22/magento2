...
/**
 * @Title("[NO TESTCASEID]: Admin user login as customer and place order")
 * @Description("Verify that admin user can place order using 'Login as customer' functionality<h3>Test files</h3>app/code/Magento/LoginAsCustomer/Test/Mftf/Test/AdminLoginAsCustomerPlaceOrderTest.xml<br>")
 * @group login_as_customer
 */
class AdminLoginAsCustomerPlaceOrderTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function AdminLoginAsCustomerPlaceOrderTest(AcceptanceTester $I)
	{
		...
		$I->amOnPage("sales/order/view/order_id/{$grabOrderId}"); // stepKey: gotoOrderPageVerifyStorefrontMessageOrderCreatedByAdmin
		...
		$I->amOnPage((getenv("MAGENTO_BACKEND_BASE_URL") ? rtrim(getenv("MAGENTO_BACKEND_BASE_URL"), "/") : "") . "/" . getenv("MAGENTO_BACKEND_NAME") . "/sales/order/view/order_id/{$grabOrderId}"); // stepKey: gotoOrderPageVerifyAdminMessageOrderCreatedByAdmin
