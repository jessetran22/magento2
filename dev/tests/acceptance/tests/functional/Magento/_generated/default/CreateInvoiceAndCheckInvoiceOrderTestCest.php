...
/**
 * @Title("MC-15868: Create invoice and check invoice order test")
 * @Description("Create invoice for offline payment methods and check invoice order on admin dashboard<h3>Test files</h3>app/code/Magento/Sales/Test/Mftf/Test/CreateInvoiceAndCheckInvoiceOrderTest.xml<br>")
 * @TestCaseId("MC-15868")
 * @group sales
 * @group mtf_migrated
 */
class CreateInvoiceAndCheckInvoiceOrderTestCest
{
    /**
	...
	public function _after(AcceptanceTester $I)
	{
		...
		$enableBankTransfer = $I->magentoCLI("config:set payment/banktransfer/active 0", 60); // stepKey: enableBankTransfer
		$I->comment($enableBankTransfer);
	 ...
	 * @throws \Exception
	 */
	public function CreateInvoiceAndCheckInvoiceOrderTest(AcceptanceTester $I)
	{
		...
		$I->comment("Entering Action Group [selectPaymentMethod] SelectBankTransferPaymentMethodActionGroup");
		$I->waitForElementVisible("#order-billing_method", 30); // stepKey: waitForPaymentOptionsSelectPaymentMethod
		$I->conditionalClick("#p_method_banktransfer", "#p_method_banktransfer", true); // stepKey: checkBankTransferOptionSelectPaymentMethod
		$I->waitForPageLoad(30); // stepKey: checkBankTransferOptionSelectPaymentMethodWaitForPageLoad
		$I->comment("Exiting Action Group [selectPaymentMethod] SelectBankTransferPaymentMethodActionGroup");
		...
		$I->click("table.data-grid tbody > tr:nth-of-type(1)"); // stepKey: clickOrderInGrid
		...
		$I->fillField("[name='grand_total[from]']", "110.00"); // stepKey: fillAmountFromFilter
		$I->waitForPageLoad(30); // stepKey: fillAmountFromFilterWaitForPageLoad
		$I->fillField("[name='grand_total[to]']", "110.00"); // stepKey: fillAmountToFilter
		...
		$I->dontSeeElement(".order-date date"); // stepKey: dontSeeDateTag
