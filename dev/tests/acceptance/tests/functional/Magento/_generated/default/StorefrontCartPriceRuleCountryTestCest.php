...
/**
 * @Title("MC-241: Customer should only see cart price rule discount if condition shipping country")
 * @Description("Customer should only see cart price rule discount if condition shipping country<h3>Test files</h3>app/code/Magento/SalesRule/Test/Mftf/Test/StorefrontCartPriceRuleCountryTest.xml<br>")
 * @TestCaseId("MC-241")
 * @group SalesRule
 */
class StorefrontCartPriceRuleCountryTestCest
{
    /**
	...
	public function _after(AcceptanceTester $I)
	{
		...
		$I->amOnPage((getenv("MAGENTO_BACKEND_BASE_URL") ? rtrim(getenv("MAGENTO_BACKEND_BASE_URL"), "/") : "") . "/" . getenv("MAGENTO_BACKEND_NAME") . "/admin/auth/logout/"); // stepKey: amOnLogoutPage
	}
	 ...
	 * @throws \Exception
	 */
	public function StorefrontCartPriceRuleCountryTest(AcceptanceTester $I)
	{
		...
		$I->selectOption("select[data-form-part='sales_rule_form'][data-ui-id='newchild-0-select-rule-conditions-1-new-child']", "Shipping Country"); // stepKey: selectProductAttributes
		...
		$I->selectOption("#conditions__1--1__value", "Brazil"); // stepKey: fillValue
		...
		$I->amOnPage($I->retrieveEntityField('createPreReqProduct', 'custom_attributes[url_key]', 'test') . ".html"); // stepKey: goToProductPage
		...
		$I->comment("Should not see the discount yet because we have not set country");
		...
		$I->click("#block-shipping-heading"); // stepKey: openEstimateShippingSection
		$I->waitForPageLoad(10); // stepKey: openEstimateShippingSectionWaitForPageLoad
		...
		$I->comment("See discount if we use valid country");
		$I->selectOption("select[name='country_id']", "Brazil"); // stepKey: fillCountry
		$I->waitForPageLoad(10); // stepKey: fillCountryWaitForPageLoad
		$I->waitForPageLoad(30); // stepKey: waitForCountry1
		...
		$I->see("-$9.99", "td[data-th='Discount']"); // stepKey: assertDiscountSeeDiscountTotal
		$I->comment("Exiting Action Group [seeDiscountTotal] AssertStorefrontCartDiscountActionGroup");
		$I->comment("Do not see discount with other country");
		$I->selectOption("select[name='country_id']", "United States"); // stepKey: fillCountry2
		$I->waitForPageLoad(10); // stepKey: fillCountry2WaitForPageLoad
		$I->waitForPageLoad(30); // stepKey: waitForCountry2
