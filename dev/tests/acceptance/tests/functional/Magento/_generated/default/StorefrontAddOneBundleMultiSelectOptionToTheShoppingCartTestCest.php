...
/**
 * @Title("MC-14727: Select one multi select option of a bundle product and add to the shopping cart")
 * @Description("Select one multi select option of a bundle product and add to the shopping cart<h3>Test files</h3>app/code/Magento/Checkout/Test/Mftf/Test/StorefrontAddOneBundleMultiSelectOptionToTheShoppingCartTest.xml<br>")
 * @TestCaseId("MC-14727")
 * @group mtf_migrated
 */
class StorefrontAddOneBundleMultiSelectOptionToTheShoppingCartTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function StorefrontAddOneBundleMultiSelectOptionToTheShoppingCartTest(AcceptanceTester $I)
	{
		$I->comment("Open Product page in StoreFront and assert product details");
		...
		$I->comment("Select One Product Option from MultiSelect option");
		$I->selectOption("//div[@class='field option  required']//select", $I->retrieveEntityField('simpleProduct2', 'name', 'test') . " +$50.00"); // stepKey: selectOption2Product
		$I->comment("Enter product Quantity and add to the cart");
		...
		$I->comment("Entering Action Group [AssertCartSummary] StorefrontCheckCartActionGroup");
		$I->seeInCurrentUrl("/checkout/cart"); // stepKey: assertUrlAssertCartSummary
		$I->waitForPageLoad(30); // stepKey: waitForCartPageAssertCartSummary
		$I->conditionalClick("#block-shipping-heading", "#co-shipping-method-form", false); // stepKey: openEstimateShippingSectionAssertCartSummary
		$I->waitForElementVisible("#s_method_flatrate_flatrate", 30); // stepKey: waitForShippingSectionAssertCartSummary
		$I->waitForPageLoad(30); // stepKey: waitForShippingSectionAssertCartSummaryWaitForPageLoad
		$I->checkOption("#s_method_flatrate_flatrate"); // stepKey: selectShippingMethodAssertCartSummary
		$I->waitForPageLoad(30); // stepKey: selectShippingMethodAssertCartSummaryWaitForPageLoad
		$I->scrollTo("//*[@id='cart-totals']//tr[@class='totals sub']//td//span[@class='price']"); // stepKey: scrollToSummaryAssertCartSummary
		$I->see("$50.00", "//*[@id='cart-totals']//tr[@class='totals sub']//td//span[@class='price']"); // stepKey: assertSubtotalAssertCartSummary
		$I->see("(Flat Rate - Fixed)", "//*[@id='cart-totals']//tr[@class='totals shipping excl']//th//span[@class='value']"); // stepKey: assertShippingMethodAssertCartSummary
		$I->waitForText("5.00", 45, "//*[@id='cart-totals']//tr[@class='totals shipping excl']//td//span[@class='price']"); // stepKey: assertShippingAssertCartSummary
		$I->see("55.00", "//*[@id='cart-totals']//tr[@class='grand totals']//td//span[@class='price']"); // stepKey: assertTotalAssertCartSummary
		$I->comment("Exiting Action Group [AssertCartSummary] StorefrontCheckCartActionGroup");
		...
		$I->see("$50.00", "//main//table[@id='shopping-cart-table']//tbody//tr[..//strong[contains(@class, 'product-item-name')]//a/text()='" . $I->retrieveEntityField('createBundleProduct', 'name', 'test') . "'][1]//td[contains(@class, 'price')]//span[@class='price']"); // stepKey: seeProductPriceInCartAssertSimpleProduct1ItemsInCheckOutCart
		$I->see("$50.00", "//main//table[@id='shopping-cart-table']//tbody//tr[..//strong[contains(@class, 'product-item-name')]//a/text()='" . $I->retrieveEntityField('createBundleProduct', 'name', 'test') . "'][1]//td[contains(@class, 'subtotal')]//span[@class='price']"); // stepKey: seeSubtotalPriceAssertSimpleProduct1ItemsInCheckOutCart
		...
		$I->see("$50.00", ".minicart-items"); // stepKey: seeProductPriceInMiniCartAssertSimpleProductInMiniCart
		...
		$I->seeElement("//*[@id='mini-cart']//a[contains(text(),'" . $I->retrieveEntityField('createBundleProduct', 'name', 'test') . "')]/../..//div[@class='details-qty qty']//input[@data-item-qty='1']"); // stepKey: seeProductQuantity1AssertSimpleProductInMiniCart
		$I->seeElement("//ol[@id='mini-cart']//img[@class='product-image-photo']"); // stepKey: seeProductImageAssertSimpleProductInMiniCart
		$I->see("$50.00", "//div[@class='subtotal']//span/span[@class='price']"); // stepKey: seeSubTotalAssertSimpleProductInMiniCart
		$I->see($I->retrieveEntityField('createBundleProduct', 'name', 'test'), ".minicart-items"); // stepKey: seeProductNameInMiniCartAssertSimpleProductInMiniCart
