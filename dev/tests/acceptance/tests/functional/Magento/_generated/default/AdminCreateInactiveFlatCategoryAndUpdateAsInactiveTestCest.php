...
/**
 * @Title("MC-11009: Flat Catalog - Update Inactive Category as Inactive, Should Not be Visible on Storefront")
 * @Description("Login as admin and create inactive flat category and update category as inactive and verify category is not visible in store front<h3>Test files</h3>app/code/Magento/Catalog/Test/Mftf/Test/AdminCreateInactiveFlatCategoryAndUpdateAsInactiveTest.xml<br>")
 * @TestCaseId("MC-11009")
 * @group mtf_migrated
 */
class AdminCreateInactiveFlatCategoryAndUpdateAsInactiveTestCest
{
    /**
	...
	public function _before(AcceptanceTester $I)
	{
		...
		$I->createEntity("createCategory", "hook", "CatNotActive", [], []); // stepKey: createCategory
	 ...
	 * @throws \Exception
	 */
	public function AdminCreateInactiveFlatCategoryAndUpdateAsInactiveTest(AcceptanceTester $I)
	{
		$I->comment("Select created category and make category inactive");
		...
		$I->click("//a/span[contains(text(), 'NotActive" . msq("CatNotActive") . "')]"); // stepKey: selectCreatedCategory
		...
		$I->see("NotActive" . msq("CatNotActive"), "h1.page-title"); // stepKey: seeUpdatedCategoryTitle
		$I->dontSeeCheckboxIsChecked("input[name='is_active']+label"); // stepKey: verifyInactiveCategory
		$I->comment("Adding the comment to replace CliIndexerReindexActionGroup action group ('indexer:reindex' commands) for preserving Backward Compatibility");
		$I->comment("Adding the comment to replace CliCacheFlushActionGroup action group ('cache:flush' command) for preserving Backward Compatibility");
		...
		$I->amOnPage("/" . $I->retrieveEntityField('createCategory', 'custom_attributes[url_key]', 'test') . ".html"); // stepKey: openCategoryPage1
		$I->waitForPageLoad(30); // stepKey: waitForCategoryStoreFrontPageToLoad
		$I->seeElement("//h1[@class='page-title']//span[contains(., 'Whoops, our bad...')]"); // stepKey: seeWhoopsOurBadMessage
		$I->comment("Verify category is not visible in First Store View");
		...
		$I->dontSeeElement("//nav//a[span[contains(., '" . $I->retrieveEntityField('createCategory', 'name', 'test') . "')]]"); // stepKey: dontSeeCategoryOnNavigation
		$I->waitForPageLoad(30); // stepKey: dontSeeCategoryOnNavigationWaitForPageLoad
		$I->comment("Verify category is not visible in Second Store View");
		...
		$I->dontSeeElement("//nav//a[span[contains(., '" . $I->retrieveEntityField('createCategory', 'name', 'test') . "')]]"); // stepKey: dontSeeCategoryOnNavigation1
		$I->waitForPageLoad(30); // stepKey: dontSeeCategoryOnNavigation1WaitForPageLoad
	}
}
