...
/**
 * @Title("MC-10824: Update Simple Product Tiered Price")
 * @Description("Test log in to Update Simple Product and Update Simple Product Tiered Price<h3>Test files</h3>app/code/Magento/Catalog/Test/Mftf/Test/AdminUpdateSimpleProductTieredPriceTest.xml<br>")
 * @TestCaseId("MC-10824")
 * @group catalog
 * @group mtf_migrated
 */
class AdminUpdateSimpleProductTieredPriceTestCest
{
    /**
	...
	public function _after(AcceptanceTester $I)
	{
		...
		$I->fillField("input.admin__control-text[name='sku']", "test_simple_product_sku" . msq("simpleProductTierPrice300InStock")); // stepKey: fillProductSkuFilterDeleteCreatedProduct
		...
		$I->see("test_simple_product_sku" . msq("simpleProductTierPrice300InStock"), "//tr[1]//td[count(//div[@data-role='grid-wrapper']//tr//th[normalize-space(.)='SKU']/preceding-sibling::th) +1 ]"); // stepKey: seeProductSkuInGridDeleteCreatedProduct
	 ...
	 * @throws \Exception
	 */
	public function AdminUpdateSimpleProductTieredPriceTest(AcceptanceTester $I)
	{
		...
		$I->comment("Comment is added to preserve the step key for backward compatibility");
		$I->comment("Update simple product with tier price(in stock)");
		$I->fillField(".admin__field[data-index=name] input", "TestSimpleProduct" . msq("simpleProductTierPrice300InStock")); // stepKey: fillSimpleProductName
		$I->fillField(".admin__field[data-index=sku] input", "test_simple_product_sku" . msq("simpleProductTierPrice300InStock")); // stepKey: fillSimpleProductSku
		$I->fillField(".admin__field[data-index=price] input", "300.00"); // stepKey: fillSimpleProductPrice
		...
		$I->fillField("[name='product[tier_price][0][price_qty]']", "1"); // stepKey: fillProductTierPriceQuantityInput
		$I->fillField("[name='product[tier_price][0][price]']", "500,000.00"); // stepKey: selectProductTierPriceFixedPrice
		...
		$I->fillField(".admin__field[data-index=qty] input", "34"); // stepKey: fillSimpleProductQuantity
		...
		$I->fillField("input[name='product[url_key]']", "test-simple-product" . msq("simpleProductTierPrice300InStock")); // stepKey: fillUrlKey
		...
		$I->fillField("input.admin__control-text[name='name']", "TestSimpleProduct" . msq("simpleProductTierPrice300InStock")); // stepKey: fillSimpleProductNameInNameFilter
		$I->fillField("input.admin__control-text[name='sku']", "test_simple_product_sku" . msq("simpleProductTierPrice300InStock")); // stepKey: fillProductSku
		...
		$I->seeInField(".admin__field[data-index=name] input", "TestSimpleProduct" . msq("simpleProductTierPrice300InStock")); // stepKey: seeSimpleProductName
		$I->seeInField(".admin__field[data-index=sku] input", "test_simple_product_sku" . msq("simpleProductTierPrice300InStock")); // stepKey: seeSimpleProductSku
		$I->seeInField(".admin__field[data-index=price] input", "300.00"); // stepKey: seeSimpleProductPrice
		...
		$I->seeInField("[name='product[tier_price][0][price_qty]']", "1"); // stepKey: seeProductTierPriceQuantityInput
		$I->seeInField("[name='product[tier_price][0][price]']", "500,000.00"); // stepKey: seeProductTierPriceFixedPrice
		...
		$I->seeInField(".admin__field[data-index=qty] input", "34"); // stepKey: seeSimpleProductQuantity
		...
		$I->comment("Comment is added to preserve the step key for backward compatibility");
		$I->comment("Entering Action Group [seeSelectedCategories] AssertAdminProductIsAssignedToCategoryActionGroup");
		$I->seeElement("//*[@class='admin__action-multiselect-crumb']/span[contains(text(), '" . $I->retrieveEntityField('categoryEntity', 'name', 'test') . "')]"); // stepKey: seeCategoryNameSeeSelectedCategories
		$I->comment("Exiting Action Group [seeSelectedCategories] AssertAdminProductIsAssignedToCategoryActionGroup");
		...
		$I->seeInField("input[name='product[url_key]']", "test-simple-product" . msq("simpleProductTierPrice300InStock")); // stepKey: seeUrlKey
		...
		$I->see("TestSimpleProduct" . msq("simpleProductTierPrice300InStock"), "a.product-item-link"); // stepKey: seeSimpleProductNameOnCategoryPage
		...
		$I->amOnPage("/test-simple-product" . msq("simpleProductTierPrice300InStock") . ".html"); // stepKey: amOnProductPageGoToProductPage
		...
		$I->see("TestSimpleProduct" . msq("simpleProductTierPrice300InStock"), ".base"); // stepKey: seeProductNameSeeSimpleProductNameOnStoreFrontPage
		...
		$I->see("300.00", ".product-info-main [data-price-type='finalPrice']"); // stepKey: seeProductPriceSeeSimpleProductPriceOnStoreFrontPage
		...
		$I->see("test_simple_product_sku" . msq("simpleProductTierPrice300InStock"), ".product.attribute.sku>.value"); // stepKey: seeProductSkuSeeProductSku
		...
		$I->amOnPage("/test-simple-product" . msq("simpleProductTierPrice300InStock") . ".html"); // stepKey: goToMagentoStorefrontPage
		...
		$I->fillField("#search", "test_simple_product_sku" . msq("simpleProductTierPrice300InStock")); // stepKey: fillInputFillSimpleProductSkuInSearchTextBox
		...
		$I->seeInTitle("Search results for: 'test_simple_product_sku" . msq("simpleProductTierPrice300InStock") . "'"); // stepKey: assertQuickSearchTitleFillSimpleProductSkuInSearchTextBox
		$I->see("Search results for: 'test_simple_product_sku" . msq("simpleProductTierPrice300InStock") . "'", ".page-title span"); // stepKey: assertQuickSearchNameFillSimpleProductSkuInSearchTextBox
		...
		$I->see("TestSimpleProduct" . msq("simpleProductTierPrice300InStock"), ".product-item-name"); // stepKey: seeProductNameSeeProductName
