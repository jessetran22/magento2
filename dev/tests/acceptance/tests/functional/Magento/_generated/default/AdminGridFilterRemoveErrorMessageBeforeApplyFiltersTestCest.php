...
/**
 * @Title("MC-37450: Remove Error Message Before Apply Filters")
 * @Description("Test login to Admin UI and Remove Error Message Before Apply Filters<h3>Test files</h3>app/code/Magento/Ui/Test/Mftf/Test/AdminGridFilterRemoveErrorMessageBeforeApplyFiltersTest.xml<br>")
 * @TestCaseId("MC-37450")
 * @group ui
 */
class AdminGridFilterRemoveErrorMessageBeforeApplyFiltersTestCest
{
    /**
	...
	public function _before(AcceptanceTester $I)
	{
		...
		$I->createEntity("createProduct2", "hook", "defaultSimpleProduct", ["rootCategory"], []); // stepKey: createProduct2
	 ...
	 * @throws \Exception
	 */
	public function AdminGridFilterRemoveErrorMessageBeforeApplyFiltersTest(AcceptanceTester $I)
	{
		$I->comment("Filter created simple product in grid and add category and website created in create data");
		$I->amOnPage((getenv("MAGENTO_BACKEND_BASE_URL") ? rtrim(getenv("MAGENTO_BACKEND_BASE_URL"), "/") : "") . "/" . getenv("MAGENTO_BACKEND_NAME") . "/catalog/product/index"); // stepKey: openProductCatalogPage
		$I->waitForPageLoad(30); // stepKey: waitForProductCatalogPage
		...
		$I->fillField("input.admin__control-text[name='sku']", $I->retrieveEntityField('createProduct2', 'sku', 'test')); // stepKey: fillProductSkuFilterFilterProduct
		...
		$I->fillField("input.admin__control-text[name='name']", $I->retrieveEntityField('createProduct2', 'name', 'test')); // stepKey: fillProductNameInNameFilterSearchCreatedSimpleProductInGrid
		...
		$I->see($I->retrieveEntityField('createProduct2', 'name', 'test'), ".data-row:nth-of-type(1)"); // stepKey: seeFirstRowToVerifyProductVisibleInGridSearchCreatedSimpleProductInGrid
		...
		$I->comment("Apply new filters to verify error message is removed");
		$I->click("button[data-action='grid-filter-expand']"); // stepKey: clickFiltersButton
		$I->click("//select[@name='store_id']/option[contains(.,'Default Store View')]"); // stepKey: clickStoreViewDropdown
		$I->fillField("input.admin__control-text[name='name']", $I->retrieveEntityField('createProduct', 'name', 'test')); // stepKey: fillProductNameInNameFilter
		...
		$I->see($I->retrieveEntityField('createProduct', 'name', 'test'), ".data-row:nth-of-type(1)"); // stepKey: seeFirstRowToVerifyProductVisibleInGrid
		$I->waitForPageLoad(30); // stepKey: seeFirstRowToVerifyProductVisibleInGridWaitForPageLoad
		$I->dontSeeElement("#messages div.message-error"); // stepKey: dontSeeErrorMessage
	}
}
