...
/**
 * @Title("MC-10823: Update Simple Product Price to Verify Data Overriding on Store View Level")
 * @Description("Test log in to Update Simple Product and Update Simple Product Price to Verify Data Overriding on Store View Level<h3>Test files</h3>app/code/Magento/Catalog/Test/Mftf/Test/AdminUpdateSimpleProductPriceToVerifyDataOverridingOnStoreViewLevelTest.xml<br>")
 * @TestCaseId("MC-10823")
 * @group catalog
 * @group mtf_migrated
 */
class AdminUpdateSimpleProductPriceToVerifyDataOverridingOnStoreViewLevelTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function AdminUpdateSimpleProductPriceToVerifyDataOverridingOnStoreViewLevelTest(AcceptanceTester $I)
	{
		...
		$I->waitForElementVisible("//div[contains(@class, 'store-switcher')]//a[normalize-space()='FR" . msq("customStoreFR") . "']", 30); // stepKey: waitForSelectCategoryStoreViewOption
		...
		$I->comment("Update default simple product with price");
		$I->fillField(".admin__field[data-index=price] input", "9.99"); // stepKey: fillSimpleProductPrice
		...
		$I->comment("Verify customer see simple product with updated price on magento storefront page");
		...
		$I->comment("Comment is added to preserve the step key for backward compatibility");
		$I->comment("Comment is added to preserve the step key for backward compatibility");
		$I->see("9.99", "//span[@class='price-wrapper ']/span[@class='price']"); // stepKey: seeUpdatedProductPriceOnStorefrontPage
		$I->comment("Verify customer see simple product with updated price on magento storefront page under store view section");
		...
		$I->comment("Comment is added to preserve the step key for backward compatibility");
		$I->comment("Comment is added to preserve the step key for backward compatibility");
		$I->see("9.99", "//span[@class='price-wrapper ']/span[@class='price']"); // stepKey: seeUpdatedProductPriceOnStorefrontPageUnderStoreViewSection
	}
}
