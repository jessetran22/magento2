...
/**
 * @Title("MC-256: Tax for Simple Product Quote should be recalculated according to inputted data on Checkout flow for Logged in Customer")
 * @Description("Tax for Simple Product Quote should be recalculated according to inputted data on Checkout flow for Logged in Customer<h3>Test files</h3>app/code/Magento/Tax/Test/Mftf/Test/StorefrontTaxQuoteCheckoutTest/StorefrontTaxQuoteCheckoutLoggedInSimpleTest.xml<br>")
 * @TestCaseId("MC-256")
 * @group Tax
 */
class StorefrontTaxQuoteCheckoutLoggedInSimpleTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function StorefrontTaxQuoteCheckoutLoggedInSimpleTest(AcceptanceTester $I)
	{
		...
		$I->see("New York", ".shipping-address-item.selected-item"); // stepKey: seeAddress
		...
		$I->waitForPageLoad(30); // stepKey: waitForAddressPopUp
		$I->comment("Entering Action Group [changeAddress] LoggedInCheckoutFillNewBillingAddressActionGroup");
		$I->fillField("._show input[name=firstname]", "John"); // stepKey: fillFirstNameChangeAddress
		...
		$I->fillField("._show input[name=telephone]", "512-345-6789"); // stepKey: fillPhoneNumberChangeAddress
		$I->fillField("._show input[name='street[0]']", "7700 West Parmer Lane"); // stepKey: fillStreetAddress1ChangeAddress
		$I->fillField("._show input[name='street[1]']", "113"); // stepKey: fillStreetAddress2ChangeAddress
		$I->fillField("._show input[name=city]", "Los Angeles"); // stepKey: fillCityNameChangeAddress
		$I->selectOption("._show select[name=region_id]", "California"); // stepKey: selectStateChangeAddress
		$I->fillField("._show input[name=postcode]", "90001"); // stepKey: fillZipChangeAddress
		$I->selectOption("._show select[name=country_id]", "US"); // stepKey: selectCountyChangeAddress
		...
		$I->comment("Entering Action Group [saveAddress] StorefrontCheckoutClickSaveAddressButtonActionGroup");
		$I->waitForElementVisible(".action-save-address", 30); // stepKey: waitForSaveButtonSaveAddress
		$I->click(".action-save-address"); // stepKey: clickSaveButtonSaveAddress
		$I->waitForPageLoad(30); // stepKey: waitForAddressSavedSaveAddress
		$I->waitForElementNotVisible("#co-shipping-form", 30); // stepKey: waitForNewAddressFormGoneSaveAddress
		$I->comment("Exiting Action Group [saveAddress] StorefrontCheckoutClickSaveAddressButtonActionGroup");
		$I->comment("Adding the comment to replace waitForAddressSaved action for preserving Backward Compatibility");
		$I->see("California", ".shipping-address-item.selected-item"); // stepKey: seeAddress2
