...
/**
 * @Title("MC-15606: Create Cart Price Rule And Verify Rule Condition Is Not Applied")
 * @Description("Test log in to Cart Price Rules and Create Cart Price Rule And Verify Rule Condition Is Not Applied<h3>Test files</h3>app/code/Magento/SalesRule/Test/Mftf/Test/AdminCreateCartPriceRuleAndVerifyRuleConditionIsNotAppliedTest.xml<br>")
 * @TestCaseId("MC-15606")
 * @group SalesRule
 * @group mtf_migrated
 */
class AdminCreateCartPriceRuleAndVerifyRuleConditionIsNotAppliedTestCest
{
    /**
	...
	public function _after(AcceptanceTester $I)
	{
		...
		$I->fillField("input[name='name']", "Cart Price Rule Condition Not Applied" . msq("CartPriceRuleConditionNotApplied")); // stepKey: filterByNameDeleteCreatedCartPriceRule
	 ...
	 * @throws \Exception
	 */
	public function AdminCreateCartPriceRuleAndVerifyRuleConditionIsNotAppliedTest(AcceptanceTester $I)
	{
		...
		$I->fillField("input[name='name']", "Cart Price Rule Condition Not Applied" . msq("CartPriceRuleConditionNotApplied")); // stepKey: fillRuleName
		...
		$I->comment("Fill condition 1: Total Items Quantity");
		$I->conditionalClick("div[data-index='conditions']", "div[data-index='conditions']", true); // stepKey: expandConditions
		$I->waitForPageLoad(30); // stepKey: expandConditionsWaitForPageLoad
		...
		$I->selectOption("select[name='rule[conditions][1][new_child]']", "Total Items Quantity"); // stepKey: selectCondition
		$I->waitForPageLoad(30); // stepKey: waitForConditionLoad
		$I->click("//ul[contains(@id, 'conditions')]//a[.='...']"); // stepKey: clickEllipsis
		$I->fillField("[id='conditions__1--1__value']", "3"); // stepKey: fillTotalItemQuantity
		...
		$I->fillField("input[name='store_labels[0]']", "Total Items Quantity-Not Applied test"); // stepKey: fillDefaultRuleLabelAllStoreViewsCreateActiveCartPriceRuleLabelsSection
		$I->fillField("input[name='store_labels[1]']", "Total Items Quantity-Not Applied test"); // stepKey: fillDefaultStoreViewCreateActiveCartPriceRuleLabelsSection
		...
		$I->fillField("input[name='name']", "Cart Price Rule Condition Not Applied" . msq("CartPriceRuleConditionNotApplied")); // stepKey: filterByNameSearchCreatedCartPriceRuleInGrid
		...
		$I->seeInField("input[name='name']", "Cart Price Rule Condition Not Applied" . msq("CartPriceRuleConditionNotApplied")); // stepKey: seeRuleName
		...
		$I->seeInField("[id='conditions__1--1__value']", "3"); // stepKey: seeTotalItemQuantity
		...
		$I->seeInField("input[name='store_labels[0]']", "Total Items Quantity-Not Applied test"); // stepKey: seeDefaultRuleLabelAllStoreViews
		$I->seeInField("input[name='store_labels[1]']", "Total Items Quantity-Not Applied test"); // stepKey: seeDefaultStoreView
		...
		$I->fillField("#qty", "2"); // stepKey: fillProductQuantity
		...
		$I->seeElement("//*[@id='mini-cart']//a[contains(text(),'" . $I->retrieveEntityField('initialSimpleProduct', 'name', 'test') . "')]/../..//div[@class='details-qty qty']//input[@data-item-qty='2']"); // stepKey: seeProductQuantity1VerifyCartSubtotalEqualsGrandTotal
		$I->seeElement("//ol[@id='mini-cart']//img[@class='product-image-photo']"); // stepKey: seeProductImageVerifyCartSubtotalEqualsGrandTotal
		$I->see("$1,120.00", "//div[@class='subtotal']//span/span[@class='price']"); // stepKey: seeSubTotalVerifyCartSubtotalEqualsGrandTotal
		...
		$I->comment("Verify AssertCartPriceRuleConditionIsNotApplied(Shopping cart subtotal equals to grand total - price rule has not been applied)");
		...
		$I->see("$1,120.00", "//*[@id='cart-totals']//tr[@class='totals sub']//td//span[@class='price']"); // stepKey: assertSubtotalCartAssert
		...
		$I->waitForText("$10.00", 45, "//*[@id='cart-totals']//tr[@class='totals shipping excl']//td//span[@class='price']"); // stepKey: assertShippingCartAssert
		$I->see("$1,130.00", "//*[@id='cart-totals']//tr[@class='grand totals']//td//span[@class='price']"); // stepKey: assertTotalCartAssert
		$I->comment("Exiting Action Group [cartAssert] StorefrontCheckCartActionGroup");
		$I->dontSee("td[data-th='Discount']"); // stepKey: dontSeeAssertDiscountAmountForRuleConditionNotApplied
	}
}
