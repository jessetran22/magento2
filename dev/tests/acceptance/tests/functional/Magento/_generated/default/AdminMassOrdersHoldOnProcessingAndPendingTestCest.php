...
/**
 * @Title("MC-16185: Mass put orders in statuses Pending, Processing on Hold")
 * @Description("Put orders in statuses Pending, Processing on Hold<h3>Test files</h3>app/code/Magento/Sales/Test/Mftf/Test/AdminMassOrdersHoldOnProcessingAndPendingTest.xml<br>")
 * @TestCaseId("MC-16185")
 * @group sales
 * @group mtf_migrated
 */
class AdminMassOrdersHoldOnProcessingAndPendingTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function AdminMassOrdersHoldOnProcessingAndPendingTest(AcceptanceTester $I)
	{
		...
		$I->comment("Entering Action Group [massActionHold] AdminTwoOrderActionOnGridActionGroup");
		$I->checkOption("//td/div[text()='{$getFirstOrderId}']/../preceding-sibling::td//input"); // stepKey: selectOrderMassActionHold
		$I->waitForPageLoad(60); // stepKey: selectOrderMassActionHoldWaitForPageLoad
		$I->waitForLoadingMaskToDisappear(); // stepKey: waitForCheckMassActionHold
		$I->checkOption("//td/div[text()='{$getSecondOrderId}']/../preceding-sibling::td//input"); // stepKey: selectSecondOrderMassActionHold
		$I->waitForPageLoad(60); // stepKey: selectSecondOrderMassActionHoldWaitForPageLoad
		$I->waitForLoadingMaskToDisappear(); // stepKey: waitForSecondCheckMassActionHold
		$I->click(".action-select-wrap > .action-select"); // stepKey: openActionsMassActionHold
		$I->waitForPageLoad(30); // stepKey: openActionsMassActionHoldWaitForPageLoad
		$I->click("(//div[contains(@class, 'action-menu-items')]//span[text()='Hold'])[1]"); // stepKey: selectActionMassActionHold
		$I->waitForPageLoad(30); // stepKey: selectActionMassActionHoldWaitForPageLoad
		$I->waitForPageLoad(30); // stepKey: waitForResultsMassActionHold
		$I->comment("Exiting Action Group [massActionHold] AdminTwoOrderActionOnGridActionGroup");
		$I->see("You have put 2 order(s) on hold."); // stepKey: assertOrderOnHoldSuccessMessage
		...
		$I->selectOption("select[name='status']", "On Hold"); // stepKey: selectOrderStatusSeeFirstOrder
		...
		$I->see("On Hold", "//tr[1]//td[count(//div[@data-role='grid-wrapper']//tr//th[contains(., 'Status')]/preceding-sibling::th) +1 ]"); // stepKey: assertFirstOrderStatus
		...
		$I->selectOption("select[name='status']", "On Hold"); // stepKey: selectOrderStatusSeeSecondOrder
		...
		$I->see("On Hold", "//tr[1]//td[count(//div[@data-role='grid-wrapper']//tr//th[contains(., 'Status')]/preceding-sibling::th) +1 ]"); // stepKey: assertSecondStatus
	}
}
