 ...
 * @group Sales
 * @group mtf_migrated
 * @Title("MC-16067: Cancel the created order with zero subtotal checkout")
 * @Description("Create an order with Zero Subtotal checkout and cancel the order<h3>Test files</h3>app/code/Magento/Sales/Test/Mftf/Test/AdminCancelTheCreatedOrderWithZeroSubtotalCheckoutTest.xml<br>")
 * @TestCaseId("MC-16067")
 */
class AdminCancelTheCreatedOrderWithZeroSubtotalCheckoutTestCest
{
    /**
	...
	{
		$I->comment("Enable Zero Subtotal Checkout");
		$enableZeroSubtotalCheckout = $I->magentoCLI("config:set payment/free/active 1", 60); // stepKey: enableZeroSubtotalCheckout
		$I->comment($enableZeroSubtotalCheckout);
		...
		$I->comment("Set Free shipping method settings");
		...
		$I->comment("Create a sales rule with fixed discount");
	 ...
	 * @throws \Exception
	 */
	public function AdminCancelTheCreatedOrderWithZeroSubtotalCheckoutTest(AcceptanceTester $I)
	{
		...
		$I->comment("Start the consumer");
		$I->comment("Entering Action Group [startMessageQueue] CliConsumerStartActionGroup");
		$startMessageQueueStartMessageQueue = $I->magentoCLI("queue:consumers:start sales.rule.update.coupon.usage --max-messages=10", 60); // stepKey: startMessageQueueStartMessageQueue
		...
		$I->comment("Refresh the page");
		$I->reloadPage(); // stepKey: refreshPageAgain
