...
/**
 * @Title("[NO TESTCASEID]: No error for minicart item edit test")
 * @Description("Already selected configurable option should be selected when configurable product is edited from minicart<h3>Test files</h3>app/code/Magento/ConfigurableProduct/Test/Mftf/Test/NoErrorForMiniCartItemEditTest.xml<br>")
 * @group ConfigurableProduct
 */
class NoErrorForMiniCartItemEditTestCest
{
    /**
	 ...
	 * @Stories({"Storefront Minicart Update"})
	 ...
	 * @throws \Exception
	 */
	public function NoErrorForMiniCartItemEditTest(AcceptanceTester $I)
	{
		...
		$I->selectOption("#product-options-wrapper .super-attribute-select", "White" . msq("colorProductAttribute1")); // stepKey: selectOptionForAddingToCart
		$I->click("button#product-addtocart-button"); // stepKey: clickAddToCart
		$I->waitForPageLoad(30); // stepKey: waitForMiniCart
		$I->comment("Edit Item in Cart");
		...
		$I->click(".action.edit"); // stepKey: clickEditCartItem
		$I->waitForPageLoad(30); // stepKey: clickEditCartItemWaitForPageLoad
		$I->comment("Check if Product Configuration is still selected");
		$I->see("White" . msq("colorProductAttribute1"), "#product-options-wrapper .super-attribute-select"); // stepKey: seeConfigurationSelected
	}
}
