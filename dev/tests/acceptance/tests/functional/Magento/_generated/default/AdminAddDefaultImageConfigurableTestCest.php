...
/**
 * @Title("MC-101: Admin should be able to add default images for a Configurable Product")
 * @Description("Admin should be able to add default images for a Configurable Product<h3>Test files</h3>app/code/Magento/ConfigurableProduct/Test/Mftf/Test/AdminAddDefaultImageConfigurableTest.xml<br>")
 * @TestCaseId("MC-101")
 * @group ConfigurableProduct
 */
class AdminAddDefaultImageConfigurableTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function AdminAddDefaultImageConfigurableTest(AcceptanceTester $I)
	{
		...
		$I->comment("Entering Action Group [assertProductImageAdminProductPage] AdminAssertProductImageOnProductPageActionGroup");
		...
		$I->waitForElementVisible("//*[@id='media_gallery_content']//img[contains(@src, 'magento-logo')]", 30); // stepKey: seeImageAssertProductImageAdminProductPage
		$I->comment("Exiting Action Group [assertProductImageAdminProductPage] AdminAssertProductImageOnProductPageActionGroup");
		$I->comment("Assert product in storefront product page");
		$I->comment("Entering Action Group [AssertProductInStorefrontProductPage] AssertProductNameAndSkuInStorefrontProductPageByCustomAttributeUrlKeyActionGroup");
		$I->comment("Go to storefront product page, assert product name and sku");
		$I->amOnPage($I->retrieveEntityField('baseConfigProductHandle', 'custom_attributes[url_key]', 'test') . ".html"); // stepKey: navigateToProductPageAssertProductInStorefrontProductPage
		$I->waitForPageLoad(30); // stepKey: waitForPageLoad2AssertProductInStorefrontProductPage
		$I->seeInTitle($I->retrieveEntityField('baseConfigProductHandle', 'name', 'test')); // stepKey: assertProductNameTitleAssertProductInStorefrontProductPage
		$I->see($I->retrieveEntityField('baseConfigProductHandle', 'name', 'test'), ".base"); // stepKey: assertProductNameAssertProductInStorefrontProductPage
		$I->see($I->retrieveEntityField('baseConfigProductHandle', 'sku', 'test'), ".product.attribute.sku>.value"); // stepKey: assertProductSkuAssertProductInStorefrontProductPage
		$I->comment("Exiting Action Group [AssertProductInStorefrontProductPage] AssertProductNameAndSkuInStorefrontProductPageByCustomAttributeUrlKeyActionGroup");
		$I->comment("Assert product image in storefront product page");
		$I->comment("Entering Action Group [assertProductImageStorefrontProductPageActionGroup] AssertProductImageStorefrontProductPage2ActionGroup");
		$I->seeInCurrentUrl("/" . $I->retrieveEntityField('baseConfigProductHandle', 'custom_attributes[url_key]', 'test') . ".html"); // stepKey: checkUrlAssertProductImageStorefrontProductPageActionGroup
		$I->waitForPageLoad(30); // stepKey: waitForPageLoadAssertProductImageStorefrontProductPageActionGroup
		$I->seeElement("//*[@class='product media']//img[contains(@src, 'magento-logo')]"); // stepKey: seeImageAssertProductImageStorefrontProductPageActionGroup
		$I->comment("Exiting Action Group [assertProductImageStorefrontProductPageActionGroup] AssertProductImageStorefrontProductPage2ActionGroup");
	}
}
