 ...
 * @group Sales
 * @group mtf_migrated
 * @Title("MC-16070: Cancel the created order with purchase order payment method")
 * @Description("Create an order using Purchase Order payment method and cancel the order<h3>Test files</h3>app/code/Magento/Sales/Test/Mftf/Test/AdminCancelTheCreatedOrderWithPurchaseOrderPaymentMethodTest.xml<br>")
 * @TestCaseId("MC-16070")
 */
class AdminCancelTheCreatedOrderWithPurchaseOrderPaymentMethodTestCest
{
    /**
	...
	public function _before(AcceptanceTester $I)
	{
		...
		$I->comment("Enable Purchase Order payment method");
		$enableCPurchaseOrderPayment = $I->magentoCLI("config:set payment/purchaseorder/active 1", 60); // stepKey: enableCPurchaseOrderPayment
		$I->comment($enableCPurchaseOrderPayment);
	...
	{
		$disablePurchaseOrderPayment = $I->magentoCLI("config:set payment/purchaseorder/active 0", 60); // stepKey: disablePurchaseOrderPayment
		$I->comment($disablePurchaseOrderPayment);
	 ...
	 * @throws \Exception
	 */
	public function AdminCancelTheCreatedOrderWithPurchaseOrderPaymentMethodTest(AcceptanceTester $I)
	{
		...
		$I->comment("Select purchase order payment method and enter purchase order number");
		$I->waitForElementVisible("#order-billing_method", 30); // stepKey: waitForPaymentOptions
		$I->checkOption("#p_method_purchaseorder"); // stepKey: selectPurchaseOrderPaymentOption
		$I->waitForPageLoad(30); // stepKey: selectPurchaseOrderPaymentOptionWaitForPageLoad
		$I->waitForElementVisible("#po_number", 30); // stepKey: waitForElementToBeVisible
		$I->fillField("#po_number", "PONumber" . msq("PurchaseOrderNumber")); // stepKey: fillPurchaseOrderNumber
		$I->click(".page-header.row"); // stepKey: clickOnHeader
