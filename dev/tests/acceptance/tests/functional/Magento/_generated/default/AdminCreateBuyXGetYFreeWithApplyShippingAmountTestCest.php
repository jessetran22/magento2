...
/**
 * @Title("[NO TESTCASEID]: Admin should be able to create a cart price rule of type Buy X get Y free enable 'Apply to Shipping Amount'")
 * @Description("Use cart price rule of type Buy X get Y free with enable 'Apply to Shipping Amount'<h3>Test files</h3>app/code/Magento/SalesRule/Test/Mftf/Test/AdminCreateBuyXGetYFreeWithApplyShippingAmountTest.xml<br>")
 * @group SalesRule
 */
class AdminCreateBuyXGetYFreeWithApplyShippingAmountTestCest
{
    /**
	...
	public function _after(AcceptanceTester $I)
	{
		...
		$I->comment("Entering Action Group [resetFilter] ClearFiltersAdminDataGridActionGroup");
		$I->conditionalClick(".admin__data-grid-header [data-action='grid-filter-reset']", ".admin__data-grid-header [data-action='grid-filter-reset']", true); // stepKey: clearExistingOrderFiltersResetFilter
		$I->waitForPageLoad(30); // stepKey: clearExistingOrderFiltersResetFilterWaitForPageLoad
		$I->comment("Exiting Action Group [resetFilter] ClearFiltersAdminDataGridActionGroup");
	 ...
	 * @throws \Exception
	 */
	public function AdminCreateBuyXGetYFreeWithApplyShippingAmountTest(AcceptanceTester $I)
	{
		$I->comment("Create a cart price rule of type Buy X get Y free");
		...
		$I->fillField("input[name='discount_amount']", "1"); // stepKey: fillDiscountAmount
		$I->fillField("input[name='discount_step']", "1"); // stepKey: fillDiscountStep
		$I->click("input[name='apply_to_shipping']+label"); // stepKey: enabledApplyDiscountToShipping
		...
		$I->comment("Entering Action Group [verifyStorefrontDiscount] VerifyDiscountAmountActionGroup");
		$I->amOnPage("testproductname" . msq("_defaultProduct") . ".html"); // stepKey: goToProductPageVerifyStorefrontDiscount
		$I->waitForPageLoad(30); // stepKey: waitForProductPageLoadVerifyStorefrontDiscount
		$I->fillField("#qty", "2"); // stepKey: fillQuantityVerifyStorefrontDiscount
		$I->click("#product-addtocart-button"); // stepKey: addProductToCartVerifyStorefrontDiscount
		$I->waitForPageLoad(60); // stepKey: addProductToCartVerifyStorefrontDiscountWaitForPageLoad
		$I->waitForPageLoad(30); // stepKey: waitForAddToCartVerifyStorefrontDiscount
		$I->waitForElementVisible("div.message-success.success.message", 30); // stepKey: waitForSuccessMessageVerifyStorefrontDiscount
		$I->amOnPage("/checkout/cart"); // stepKey: goToCartPageVerifyStorefrontDiscount
		$I->waitForPageLoad(30); // stepKey: waitForCartPageVerifyStorefrontDiscount
		$I->waitForElementVisible("td[data-th='Discount']", 30); // stepKey: waitForDiscountElementVerifyStorefrontDiscount
		$I->see("-$128.00", "td[data-th='Discount']"); // stepKey: seeDiscountTotalVerifyStorefrontDiscount
		$I->comment("Exiting Action Group [verifyStorefrontDiscount] VerifyDiscountAmountActionGroup");
