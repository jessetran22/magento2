...
/**
 * @Title("MC-225: Admin should be able to save and publish a bundle product")
 * @Description("Admin should be able to save and publish a bundle product<h3>Test files</h3>app/code/Magento/Bundle/Test/Mftf/Test/AdminProductBundleCreationTest.xml<br>")
 * @TestCaseId("MC-225")
 * @group Bundle
 */
class AdminProductBundleCreationTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function AdminProductBundleCreationTest(AcceptanceTester $I)
	{
		...
		$I->comment("Create a bundle product with ancillary data");
		$I->comment("Entering Action Group [createBundledProduct] CreateBasicBundleProductActionGroup");
		$I->comment("PreReq: Go to bundle product creation page");
		$I->comment("Product name and SKU");
		$I->fillField("//*[@name='product[name]']", "BundleProduct" . msq("BundleProduct")); // stepKey: fillProductNameCreateBundledProduct
		$I->fillField("//*[@name='product[sku]']", "bundleproduct" . msq("BundleProduct")); // stepKey: fillProductSkuCreateBundledProduct
		$I->comment("Trigger SEO drop down");
		$I->scrollTo("//div[@data-index='search-engine-optimization']"); // stepKey: scrollToSeoDropDownCreateBundledProduct
		$I->conditionalClick("//div[@data-index='search-engine-optimization']", "//input[@name='product[url_key]']", false); // stepKey: openDropDownIfClosedCreateBundledProduct
		$I->waitForPageLoad(30); // stepKey: openDropDownIfClosedCreateBundledProductWaitForPageLoad
		$I->waitForPageLoad(30); // stepKey: waitForDropDownSEOCreateBundledProduct
		$I->comment("Fill URL input");
		$I->fillField("//input[@name='product[url_key]']", "bundleproduct" . msq("BundleProduct")); // stepKey: fillsInSeoLinkExtensionCreateBundledProduct
		$I->waitForPageLoad(30); // stepKey: fillsInSeoLinkExtensionCreateBundledProductWaitForPageLoad
		$I->comment("Exiting Action Group [createBundledProduct] CreateBasicBundleProductActionGroup");
		...
		$I->comment("Test Assertion - on correct page/page has been published");
