...
/**
 * @Title("[NO TESTCASEID]: Admin user directly login into customer account with store View To Login In = Manual Choose when store code is added to url")
 * @Description("Verify admin user can directly login into customer account to Custom store view when Store View To Login In = Manual Choose when store code is added to url<h3>Test files</h3>app/code/Magento/LoginAsCustomer/Test/Mftf/Test/AdminLoginAsCustomerManualChooseStoreCodeInUrlTest.xml<br>")
 * @group login_as_customer
 */
class AdminLoginAsCustomerManualChooseStoreCodeInUrlTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function AdminLoginAsCustomerManualChooseStoreCodeInUrlTest(AcceptanceTester $I)
	{
		...
		$I->comment("Entering Action Group [seeCustomStoreViewCodeInUrl] AssertStorefrontStoreCodeInUrlActionGroup");
		$I->seeInCurrentUrl("/en" . msq("customStoreEN")); // stepKey: seeStoreCodeInURLSeeCustomStoreViewCodeInUrl
		$I->comment("Exiting Action Group [seeCustomStoreViewCodeInUrl] AssertStorefrontStoreCodeInUrlActionGroup");
