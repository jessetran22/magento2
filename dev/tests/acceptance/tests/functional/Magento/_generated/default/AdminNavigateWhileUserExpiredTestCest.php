...
/**
 * @Title(": Navigate to an admin page after user expiration date passes")
 * @Description("Navigate to an admin page after user expiration date passes.<h3>Test files</h3>app/code/Magento/Security/Test/Mftf/Test/AdminNavigateWhileUserExpiredTest.xml<br>")
 * @TestCaseId("")
 * @group security
 */
class AdminNavigateWhileUserExpiredTestCest
{
    /**
	...
	/**
	 * @Features({"Security"})
	 * @Stories({"Navigate to an admin page after user expiration date passes."})
	 ...
	 * @throws \Exception
	 */
	public function AdminNavigateWhileUserExpiredTest(AcceptanceTester $I)
	{
		...
		$date = new \DateTime();
		$date->setTimestamp(strtotime("+2 minute"));
		...
		$I->click("#page_tabs_main_section"); // stepKey: openUserInfoTab
		...
		$I->wait(120); // stepKey: waitForUserToExpire
		...
		$I->comment("Confirm that user is logged out");
