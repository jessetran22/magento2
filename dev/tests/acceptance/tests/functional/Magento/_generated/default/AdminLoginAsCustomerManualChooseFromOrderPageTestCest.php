...
/**
 * @Title("[NO TESTCASEID]: Admin user directly login into customer account with store View To Login In = Manual Choose on Order Page")
 * @Description("Verify admin user can directly login into customer account to Custom store view when Store View To Login In = Manual Choose on Order Page<h3>Test files</h3>app/code/Magento/LoginAsCustomer/Test/Mftf/Test/AdminLoginAsCustomerManualChooseFromOrderPageTest.xml<br>")
 * @group login_as_customer
 */
class AdminLoginAsCustomerManualChooseFromOrderPageTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function AdminLoginAsCustomerManualChooseFromOrderPageTest(AcceptanceTester $I)
	{
		$I->comment("Create order");
		$I->comment("Entering Action Group [createOrder] CreateOrderInStoreActionGroup");
		...
		$I->click("//div[contains(@class, 'tree-store-scope')]//label[contains(text(), 'FR" . msq("customStoreFR") . "')]"); // stepKey: chooseStoreCreateOrder
		$I->waitForPageLoad(30); // stepKey: chooseStoreCreateOrderWaitForPageLoad
		$I->scrollToTopOfPage(); // stepKey: scrollToTopCreateOrder
		$I->waitForPageLoad(30); // stepKey: waitForStoreToAppearCreateOrder
		...
		$I->comment("Exiting Action Group [createOrder] CreateOrderInStoreActionGroup");
		$grabOrderId = $I->grabFromCurrentUrl("~/order_id/(\d+)/~"); // stepKey: grabOrderId
		$I->comment("Login as Customer from Order page");
		$I->comment("Entering Action Group [loginAsCustomerFromOrderPage] AdminLoginAsCustomerLoginFromOrderPageManualChooseActionGroup");
		$I->amOnPage((getenv("MAGENTO_BACKEND_BASE_URL") ? rtrim(getenv("MAGENTO_BACKEND_BASE_URL"), "/") : "") . "/" . getenv("MAGENTO_BACKEND_NAME") . "/sales/order/view/order_id/{$grabOrderId}"); // stepKey: gotoOrderPageLoginAsCustomerFromOrderPage
		$I->waitForPageLoad(30); // stepKey: waitForCustomerPageLoadLoginAsCustomerFromOrderPage
		$I->click("#guest_to_customer"); // stepKey: clickLoginAsCustomerLinkLoginAsCustomerFromOrderPage
		$I->waitForPageLoad(30); // stepKey: clickLoginAsCustomerLinkLoginAsCustomerFromOrderPageWaitForPageLoad
		$I->see("Login as Customer: Select Store", "aside.confirm .modal-title"); // stepKey: seeModalLoginAsCustomerFromOrderPage
		$I->see("Actions taken while in \"Login as Customer\" will affect actual customer data.", "aside.confirm .modal-content"); // stepKey: seeModalMessageLoginAsCustomerFromOrderPage
		$I->selectOption("//select[@id='lac-confirmation-popup-store-id']", "store" . msq("customStoreGroup")); // stepKey: selectStoreLoginAsCustomerFromOrderPage
		$I->click("aside.confirm .modal-footer button.action-accept"); // stepKey: clickLoginLoginAsCustomerFromOrderPage
		$I->waitForPageLoad(60); // stepKey: clickLoginLoginAsCustomerFromOrderPageWaitForPageLoad
		$I->switchToNextTab(); // stepKey: switchToNewTabLoginAsCustomerFromOrderPage
		$I->waitForPageLoad(30); // stepKey: waitForPageLoadLoginAsCustomerFromOrderPage
		$I->comment("Exiting Action Group [loginAsCustomerFromOrderPage] AdminLoginAsCustomerLoginFromOrderPageManualChooseActionGroup");
