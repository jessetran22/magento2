...
/**
 * @Title("MC-87: admin should be able to delete a configurable product")
 * @Description("admin should be able to delete a configurable product<h3>Test files</h3>app/code/Magento/ConfigurableProduct/Test/Mftf/Test/AdminConfigurableProductDeleteTest/AdminConfigurableProductDeleteTest.xml<br>")
 * @TestCaseId("MC-87")
 * @group ConfigurableProduct
 */
class AdminConfigurableProductDeleteTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function AdminConfigurableProductDeleteTest(AcceptanceTester $I)
	{
		$I->comment("assert product visible in storefront");
		$I->amOnPage($I->retrieveEntityField('createConfigProduct', 'custom_attributes[url_key]', 'test') . ".html"); // stepKey: gotoStorefront1
		$I->waitForPageLoad(30); // stepKey: wait1
		$I->see($I->retrieveEntityField('createConfigProduct', 'name', 'test'), ".base"); // stepKey: seeProduct
		$I->comment("go to admin and delete");
		...
		$I->click("label.data-grid-checkbox-cell-inner"); // stepKey: clickCheckbox
		$I->click("div.admin__data-grid-header-row.row div.action-select-wrap button.action-select"); // stepKey: clickActionDropdown
		$I->waitForPageLoad(30); // stepKey: clickActionDropdownWaitForPageLoad
		...
		$I->waitForPageLoad(30); // stepKey: clickDeleteActionWaitForPageLoad
		...
		$I->see("A total of 1 record(s) have been deleted.", ".message.message-success.success"); // stepKey: seeSuccessMsg
		$I->comment("after delete, assert product page is 404");
		$I->amOnPage($I->retrieveEntityField('createConfigProduct', 'custom_attributes[url_key]', 'test') . ".html"); // stepKey: gotoStorefront2
		$I->waitForPageLoad(30); // stepKey: wait3
		$I->see("Whoops, our bad...", ".base"); // stepKey: seeWhoops
		$I->dontSee($I->retrieveEntityField('createConfigProduct', 'name', 'test'), ".base"); // stepKey: dontSeeProduct
	}
}
