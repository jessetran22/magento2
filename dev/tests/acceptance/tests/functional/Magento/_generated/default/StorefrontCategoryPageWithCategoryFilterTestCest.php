...
/**
 * @Title("[NO TESTCASEID]: Category with Layered Navigation - verify presence of category filter")
 * @Description("Verify that the category filter is present in layered navigation on category page<h3>Test files</h3>app/code/Magento/Catalog/Test/Mftf/Test/StorefrontCategoryPageWithCategoryFilterTest.xml<br>")
 * @group Catalog
 */
class StorefrontCategoryPageWithCategoryFilterTestCest
{
    /**
	...
	public function _before(AcceptanceTester $I)
	{
		...
		$I->comment("Set the category filter to be present on the category page layered navigation");
		$setCategoryFilterVisibleOnStorefront = $I->magentoCLI("config:set catalog/layered_navigation/display_category 1", 60); // stepKey: setCategoryFilterVisibleOnStorefront
		$I->comment($setCategoryFilterVisibleOnStorefront);
	...
	/**
	 * @Stories({"Category page: Layered Navigation with category filter"})
	 * @Features({"Catalog"})
	 ...
	 * @throws \Exception
	 */
	public function StorefrontCategoryPageWithCategoryFilterTest(AcceptanceTester $I)
	{
		...
		$I->comment("Entering Action Group [checkCategoryFilterIsPresent] AssertStorefrontLayeredNavigationCategoryFilterVisibleActionGroup");
		$I->comment("Verify category filter item is present");
		$I->see("Category", "#layered-filter-block"); // stepKey: seeCategoryFilterInLayeredNavCheckCategoryFilterIsPresent
		$I->comment("Exiting Action Group [checkCategoryFilterIsPresent] AssertStorefrontLayeredNavigationCategoryFilterVisibleActionGroup");
	}
}
