...
/**
 * @Title("MC-5324: Create tax rule, with customer and product tax class")
 * @Description("Test log in to Tax Rule and Create tax rule with customer and product tax class<h3>Test files</h3>app/code/Magento/Tax/Test/Mftf/Test/AdminCreateTaxRuleWithCustomerAndProductTaxClassTest.xml<br>")
 * @TestCaseId("MC-5324")
 * @group tax
 * @group mtf_migrated
 */
class AdminCreateTaxRuleWithCustomerAndProductTaxClassTestCest
{
    /**
	 ...
	 * @throws \Exception
	 */
	public function AdminCreateTaxRuleWithCustomerAndProductTaxClassTest(AcceptanceTester $I)
	{
		...
		$I->comment("Create a tax rule with customer and product class");
		...
		$I->fillField("input[data-role='advanced-select-text']", "US-NY-*-Rate 1"); // stepKey: fillTaxRateSearch2
		$I->wait(5); // stepKey: waitForSearch3
		$I->conditionalClick("//*[@data-ui-id='tax-rate-form-fieldset-element-form-field-tax-rate']//span[.='US-NY-*-Rate 1']", "//span[contains(., 'US-NY-*-Rate 1') and preceding-sibling::input[contains(@class, 'mselect-checked')]]", false); // stepKey: clickSelectNeededItem2
		...
		$I->fillField("#position", "1"); // stepKey: fillSortOrder
		...
		$I->comment("Verify we see created tax rule with customer and product class(from the above step) on the tax rule grid page");
		...
		$I->comment("Verify we see created tax rule with customer and product class on the tax rule form page");
		...
		$I->seeElement("//span[contains(., 'US-NY-*-Rate 1') and preceding-sibling::input[contains(@class, 'mselect-checked')]]"); // stepKey: seeSelectNeededItem2
		...
		$I->seeInField("#position", "1"); // stepKey: seeSortOrder
	}
}
